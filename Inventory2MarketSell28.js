    ...
    methods: {
        ...
        async sell() {
            ...
            switch (e.code) {
                case 0:
                    dialog({
                        text: [`Предмет успешно выставлен на продажу на {link:/market/thing/${this.item_proto_id};Маркет}.`, "Он пока будет недоступен в Вашем инвентаре.",`${this.item.title} за ${this.price_new.total}. Получим: ${this.price_new.receive} руб.`]
                    }),
                        this.$emit("return", !0);
                    break;
                case 191:
