...
exports.default = {
    ...
    computed: {
        ...
        button_color() {
            // console.log('рассчет цвета кнопки',this.price_new.total < ((this.price_best - this.price_best * this.min_sell_limit_warn) >= 0.01 ? this.price_best * this.min_sell_limit_warn : this.price_best - 0.01) ? "red" : "grass");
            if (this.price_new.total < (this.price_best - this.price_best * this.min_sell_limit_warn) >= 0.01 ? this.price_best * this.min_sell_limit_warn : this.price_best - 0.01){
                console.log('red');
                this.price_new.total && (this.$refs.buttons.$el.style.background="red");
            } else {
                console.log('green');
                this.price_new.total && (this.$refs.buttons.$el.style.background="");
            }
            window.bb=this.$refs.buttons;
            // console.log(this.$refs.buttons);
            
            return this.price_new.total < ((this.price_best - this.price_best * this.min_sell_limit_warn) >= 0.01 ? this.price_best * this.min_sell_limit_warn : this.price_best - 0.01) ? "Внимание демпинг!!! "+this.min_sell_limit : "Выставить на продажу"
